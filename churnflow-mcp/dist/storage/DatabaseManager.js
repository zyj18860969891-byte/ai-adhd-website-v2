...
import { eq, desc, asc, and, or, isNull, lt, lte, sql, like } from 'drizzle-orm';
import { migrate } from 'drizzle-orm/better-sqlite3/migrator';
import path from 'path';
import fs from 'fs';
import { captures, contexts, learningPatterns, preferences, } from './schema.js';
export class DatabaseManager {
    ...
    async getCapturesNeedingReview(limit = 20) {
        ...
        // Due soon
        and(lte(captures.dueDate, threeDaysFromNow.toISOString()), isNull(captures.completedAt)), 
        // High priority and stale
        and(sql `${captures.priority} IN ('critical', 'high')`, lt(captures.lastReviewedAt, oneWeekAgo.toISOString())), 
        // Active but stale
        and(eq(captures.status, 'active'), lt(captures.lastReviewedAt, twoWeeksAgo.toISOString()))))
            .orderBy(desc(captures.priority), asc(captures.lastReviewedAt), asc(captures.dueDate))
            .limit(limit);
    }
    async markCaptureReviewed(id, reviewNotes) {
        const result = await this.db
            .update(captures)
            .set({
    ...
    async getNextActions(limit = 10) {
        const now = new Date().toISOString();
        return this.db
            .select()
            .from(captures)
            .where(and(eq(captures.status, 'active'), or(isNull(captures.reminderDate), lte(captures.reminderDate, now)), 
        // Must be reviewed
        sql `${captures.lastReviewedAt} IS NOT NULL`))
            .orderBy(desc(captures.priority), asc(captures.dueDate), asc(captures.createdAt))
            .limit(limit);
    }
    async getQuickWins(limit = 5) {
        return this.db
            .select()
            .from(captures)
            .where(and(eq(captures.status, 'active'), sql `LENGTH(${captures.item}) < 100`, sql `${captures.priority} IN ('low', 'medium')`, sql `${captures.lastReviewedAt} IS NOT NULL`))
            .orderBy(asc(captures.createdAt))
            .limit(limit);
    }
    async getHighImpactTasks(limit = 5) {
        return this.db
            .select()
            .from(captures)
            .where(and(eq(captures.status, 'active'), sql `${captures.priority} IN ('high', 'critical')`, sql `${captures.lastReviewedAt} IS NOT NULL`))
            .orderBy(desc(captures.priority), asc(captures.dueDate))
            .limit(limit);
    }
    // CONTEXT OPERATIONS
    ...
    // SEARCH OPERATIONS
    async searchCaptures(query, limit = 20) {
        try {
      ...
      `).all(query, limit);
        ...
        return this.db
            .select()
            .from(captures)
            .where(or(like(captures.item, `%${query}%`), like(captures.tags, `%${query}%`), like(captures.keywords, `%${query}%`)))
            .limit(limit);
    }
    // LEARNING OPERATIONS
    ...
    async updateLearningFeedback(patternId, wasCorrect, userCorrectedContextId, userCorrectedType) {
        await this.db
    ...
    async getDashboardStats() {
        ...
        const [stats] = await this.db
            .select({
            inbox: sql `COUNT(CASE WHEN status = 'inbox' THEN 1 END)`,
            active: sql `COUNT(CASE WHEN status = 'active' THEN 1 END)`,
            completed: sql `COUNT(CASE WHEN status = 'completed' THEN 1 END)`,
            needingReview: sql `COUNT(CASE WHEN last_reviewed_at IS NULL THEN 1 END)`,
            overdue: sql `COUNT(CASE WHEN due_date < ${now} AND status != 'completed' THEN 1 END)`,
        })
            .from(captures);
    ...
    async seedInitialData() {
        ...
        const defaultPrefs = [
            { key: 'review_batch_size', value: '10', type: 'number', category: 'review' },
            { key: 'confidence_threshold', value: '0.7', type: 'number', category: 'ai' },
            { key: 'color_output', value: 'true', type: 'boolean', category: 'ui' },
        ];
