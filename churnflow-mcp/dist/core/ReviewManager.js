...
export class ReviewManager {
     ...
     */
    flagItemForReview(content, confidence, tracker, section, source, metadata = {}) {
     ...
     */
    getItemsNeedingReview(tracker) {
        const items = Array.from(this.reviewItems.values());
        if (tracker) {
            return items.filter((item) => item.currentTracker === tracker &&
                (item.reviewStatus === "pending" || item.reviewStatus === "flagged"));
        }
        return items.filter((item) => item.reviewStatus === "pending" || item.reviewStatus === "flagged");
    }
    /**
     ...
     */
    async processReviewAction(itemId, action, newValues) {
    ...
    async acceptItem(item) {
        try {
            ...
            const formattedContent = FormattingUtils.formatEntry(item.metadata.type, item.content, {
            ...
            const success = await this.trackerManager.appendToTracker(item.currentTracker, formattedContent);
            if (success) {
     ...
     */
    async batchProcessReview(actions) {
        const results = [];
        let successCount = 0;
        let failedCount = 0;
     ...
     */
    clearConfirmedItems() {
        const confirmedItems = Array.from(this.reviewItems.entries()).filter(([_, item]) => item.reviewStatus === "confirmed");
