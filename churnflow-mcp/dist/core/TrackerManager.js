...
export class TrackerManager {
     ...
     */
    getTrackersByContext(contextType) {
        if (!contextType) {
            return Array.from(this.trackers.values());
        }
        return Array.from(this.trackers.values()).filter((tracker) => tracker.frontmatter.contextType === contextType);
    }
    /**
     ...
     */
    findSection(lines, sectionHeader) {
     ...
     */
    async appendToTracker(tag, formattedEntry) {
        ...
        try {
            ...
            const updatedLines = this.insertEntryIntoSection(parsed.content.split("\n"), "## Action Items", formattedEntry);
     ...
     */
    async appendActivityToTracker(tag, formattedEntry) {
        const tracker = this.trackers.get(tag);
        if (!tracker) {
            console.error(`Tracker not found: ${tag}`);
            return false;
        ...
        try {
            ...
            const parsed = matter(currentContent);
            // Use the new section placement logic
            const updatedLines = this.insertEntryIntoSection(parsed.content.split("\n"), "## Activity Log", formattedEntry, true);
            // Reconstruct the file with frontmatter
            const updatedContent = matter.stringify(updatedLines.join("\n"), parsed.data);
            // Write back to file
            await fs.writeFile(tracker.filePath, updatedContent, "utf-8");
     ...
     */
    async appendReviewToTracker(tag, formattedEntry) {
        const tracker = this.trackers.get(tag);
        if (!tracker) {
            console.error(`Tracker not found: ${tag}`);
            return false;
        ...
        try {
            ...
            const parsed = matter(currentContent);
            // Use the new section placement logic
            const updatedLines = this.insertEntryIntoSection(parsed.content.split("\n"), "## Review Queue", formattedEntry, false);
            // Reconstruct the file with frontmatter
            const updatedContent = matter.stringify(updatedLines.join("\n"), parsed.data);
            // Write back to file
            await fs.writeFile(tracker.filePath, updatedContent, "utf-8");
     ...
     */
    findSectionIndex(lines, sectionHeader) {
     ...
     */
    insertEntryIntoSection(lines, sectionHeader, newEntry, sortByDate = false) {
     ...
     */
    insertIntoExistingSection(lines, sectionIndex, newEntry, sortByDate) {
     ...
     */
    createSectionAndInsert(lines, sectionHeader, newEntry) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.startsWith("##")) {
                ...
                if (existingSectionPriority > currentSectionPriority ||
     ...
     */
    createFormattedEntry(itemType, description, options = {}) {
     ...
     */
    async appendFormattedEntry(tag, itemType, description, options = {}) {
     ...
     */
    async validateTrackerFormatting(tag) {
        const tracker = this.trackers.get(tag);
        if (!tracker) {
            return {
                isValid: false,
        ...
        const sections = [
            { header: "## Action Items", expectedType: "action" },
            { header: "## Activity Log", expectedType: "activity" },
            { header: "## Review Queue", expectedType: "review" },
            { header: "## Someday/Maybe", expectedType: "someday" },
        ];
     ...
     */
    async markTaskComplete(tracker, taskDescription) {
        try {
