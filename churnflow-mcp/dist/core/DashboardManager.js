...
export class DashboardManager {
    ...
    reviewManager;
    config;
    constructor(config, trackerManager, reviewManager) {
        this.config = config;
    ...
    async getWhatsNext() {
        ...
        const quickWin = this.getBestQuickWin(allItems.filter((item) => item.id !== urgent?.id && item.id !== highImpact?.id));
        if (quickWin) {
            recommendations.push({
                category: "quick-win",
     ...
     */
    parseTrackerForActionItems(tracker) {
        const items = [];
     ...
     */
    parseActionItem(line, tracker, section) {
        try {
     ...
     */
    extractPriorityFromObsidian(line) {
        // Obsidian Tasks priority tags
        if (line.includes("@urgent") || line.includes("@high"))
            return "high";
        ...
        if (/urgent|asap|emergency|critical/i.test(line))
            return "high";
     ...
     */
    calculateUrgencyScore(priority, dueDate) {
        let score = 0;
        // Base priority score
        switch (priority) {
            case "high":
     ...
     */
    calculateImpactScore(context, priority) {
        let score = 0;
        // Context impact
        switch (context) {
            case "business":
    ...
    estimateMinutes(title, priority) {
        ...
        if (isQuick)
            ...
            baseMinutes = 45;
        // Adjust for priority
        if (priority === "high" && baseMinutes < 30)
            baseMinutes = 30;
        return baseMinutes;
    }
    /**
     ...
     */
    getMostUrgentItem(items) {
        if (items.length === 0)
            return null;
        return items.reduce((most, current) => current.urgencyScore > most.urgencyScore ? current : most);
    }
    /**
     * Get the highest impact item
     */
    getHighestImpactItem(items) {
        if (items.length === 0)
            return null;
        return items.reduce((highest, current) => current.impactScore > highest.impactScore ? current : highest);
    }
    /**
     * Get the best quick win (low time, decent impact)
     */
    getBestQuickWin(items) {
        ...
        if (workableItems.length === 0)
            return null;
        const quickWins = workableItems.filter((item) => (item.estimatedMinutes || 30) <= 15 && item.impactScore >= 30);
        if (quickWins.length === 0) {
            // Fallback to shortest workable task
            return (workableItems.reduce((shortest, current) => (current.estimatedMinutes || 30) < (shortest.estimatedMinutes || 30)
                ? current
                : shortest, workableItems[0]) || null);
        }
        return quickWins.reduce((best, current) => current.impactScore > best.impactScore ? current : best);
    }
    /**
     * Get review chunk if items need review
     */
    async getReviewChunk() {
