...
// 带超时和重试的工具调用包装器
async function callToolWithTimeoutAndRetry(toolFn, toolName, timeoutMs = 180000, // 3分钟默认超时
maxRetries = 2) {
    let lastError = null;
            ...
            try {
                ...
                switch (toolName) {
                    ...
                    case "split_tasks":
                        ...
                        return await callToolWithTimeoutAndRetry(() => splitTasksRaw(parsedArgs.data), toolName, 300000, // 5分钟
                        3 // 最多3次重试
                        );
                    case "list_tasks":
