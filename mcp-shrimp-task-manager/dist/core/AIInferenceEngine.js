...
Please provide a comprehensive analysis following the structure above. Focus on actionable insights that help the user succeed with ADHD.`;
     ...
     */
    async generateResearch(topic, currentState, nextSteps) {
        try {
...
5. Provides actionable next steps`;
            ...
            return (response.choices[0]?.message?.content || "Research completed");
        }
        catch (error) {
     ...
     */
    async verifyTaskCompletion(taskSummary, score) {
        try {
...
5. Learning points for future similar tasks`;
            ...
            return (response.choices[0]?.message?.content || "Verification completed");
        }
        catch (error) {
     ...
     */
    async reflectOnTask(taskDescription, executionNotes) {
        try {
...
5. Specific improvements for next time`;
            ...
            return (response.choices[0]?.message?.content || "Reflection completed");
        }
        catch (error) {
