...
async function initGitIfNeeded(dataDir) {
...
async function commitTaskChanges(dataDir, message, details) {
    try {
...
// 寫入所有任務
// Write all tasks
async function writeTasks(tasks, commitMessage) {
    await ensureDataDir();
    const TASKS_FILE = await getTasksFilePath();
    const DATA_DIR = await getDataDir();
...
// 創建新任務
// Create new task
export async function createTask(name, description, notes, dependencies = [], relatedFiles, agent) {
    const tasks = await readTasks();
