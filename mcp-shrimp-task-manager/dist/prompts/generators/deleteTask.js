 ...
 */
export async function getDeleteTaskPrompt(params) {
    const { taskId, task, success, message, isTaskCompleted } = params;
    // 處理任務不存在的情況
    // Handle case when task does not exist
    if (!task) {
        const notFoundTemplate = await loadPromptFromTemplate("deleteTask/notFound.md");
    ...
    if (isTaskCompleted) {
        const completedTemplate = await loadPromptFromTemplate("deleteTask/completed.md");
