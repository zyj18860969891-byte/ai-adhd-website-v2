 ...
 */
export async function getClearAllTasksPrompt(params) {
    const { confirm, success, message, backupFile, isEmpty } = params;
    // 處理未確認的情況
    // Handle unconfirmed situations
    if (confirm === false) {
        const cancelTemplate = await loadPromptFromTemplate("clearAllTasks/cancel.md");
        return generatePrompt(cancelTemplate, {});
    }
    // 處理無任務需要清除的情況
    // Handle situations where no tasks need to be cleared
    if (isEmpty) {
        const emptyTemplate = await loadPromptFromTemplate("clearAllTasks/empty.md");
    ...
    const backupInfo = backupFile
        ? generatePrompt(await loadPromptFromTemplate("clearAllTasks/backupInfo.md"), {
            backupFile,
        })
        : "";
