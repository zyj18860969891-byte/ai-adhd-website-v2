...
function createTestTask(name, description, notes) {
    return {
        id: 'test-id',
        name,
...
describe('agentMatcher', () => {
    describe('matchAgentToTask', () => {
        ...
        it('should match frontend tasks correctly', () => {
            const task = createTestTask('Update React component', 'Need to update the UserProfile component to use new CSS styles and improve responsive design');
            expect(matchAgentToTask(task, agents)).toBe('Frontend Developer');
        });
        it('should match backend tasks correctly', () => {
            const task = createTestTask('Create REST API endpoint', 'Implement a new Express.js endpoint for user authentication using JWT tokens');
            expect(matchAgentToTask(task, agents)).toBe('Backend Engineer');
        });
        it('should match database tasks correctly', () => {
            const task = createTestTask('Optimize database queries', 'Need to add indexes to PostgreSQL tables and optimize slow SQL queries');
            expect(matchAgentToTask(task, agents)).toBe('Database Admin');
        });
        it('should match fullstack tasks correctly', () => {
            const task = createTestTask('Deploy application to AWS', 'Set up CI/CD pipeline with Docker and Kubernetes for the MERN stack application');
            expect(matchAgentToTask(task, agents)).toBe('Full Stack Developer');
        });
        it('should match mobile tasks correctly', () => {
            const task = createTestTask('Fix iOS app crash', 'Debug and fix React Native app crash on iOS devices when using camera');
            expect(matchAgentToTask(task, agents)).toBe('Mobile Developer');
        });
        it('should match testing tasks correctly', () => {
            const task = createTestTask('Write unit tests', 'Create Jest unit tests for the new authentication service with mocking');
            expect(matchAgentToTask(task, agents)).toBe('QA Engineer');
        });
        it('should match security tasks correctly', () => {
            const task = createTestTask('Security audit', 'Perform OWASP security audit and fix XSS vulnerabilities in the application');
            expect(matchAgentToTask(task, agents)).toBe('Security Expert');
        });
        it('should match data/ML tasks correctly', () => {
            const task = createTestTask('Implement ML model', 'Create a machine learning model using TensorFlow for data analysis and visualization');
        ...
        it('should return undefined for tasks with no matching keywords', () => {
            const task = createTestTask('Generic task', 'This is a task about something completely unrelated to programming');
            expect(matchAgentToTask(task, agents)).toBeUndefined();
        });
        it('should handle null/undefined task gracefully', () => {
            expect(matchAgentToTask(null, agents)).toBeUndefined();
            expect(matchAgentToTask(undefined, agents)).toBeUndefined();
        });
        ...
        it('should handle multiple keyword matches correctly', () => {
            const task = createTestTask('Full stack development', 'Build React frontend with Node.js backend API and MongoDB database integration');
            expect(matchAgentToTask(task, agents)).toBe('Full Stack Developer');
        });
    });
    ...
    describe('getKeywordMatchDetails', () => {
        it('should return detailed keyword matches', () => {
            const task = createTestTask('React and Node.js task', 'Build a React frontend with Node.js backend');
        ...
        it('should sort by score descending', () => {
            const task = createTestTask('Database optimization', 'Optimize PostgreSQL queries and add indexes to tables');
            const details = getKeywordMatchDetails(task);
            expect(details.length).toBeGreaterThan(0);
    ...
    describe('keyword scoring', () => {
        it('should give higher scores for multiple keyword matches', () => {
            ...
            const task2 = createTestTask('Complex React task', 'Update React components with TypeScript, add CSS animations, and improve responsive design');
        ...
        it('should match whole words only', () => {
            const task = createTestTask('Database optimization', 'This is about database, not about databasesystem');
            const details = getKeywordMatchDetails(task);
            ...
            const task2 = createTestTask('Preact component', 'This is about Preact component');
            const details2 = getKeywordMatchDetails(task2);
...
//# sourceMappingURL=agentMatcher.test.js.map