 ...
 */
function calculateKeywordScore(text, keywords, weight, agentType) {
 ...
 */
function findBestAgentType(task) {
 ...
 */
export function matchAgentToTask(task, availableAgents) {
    // Handle edge cases
    ...
    let matchedAgent = availableAgents.find(agent => agent.name.toLowerCase().includes(bestAgentType.type.toLowerCase()));
    if (matchedAgent) {
    ...
    const generalAgent = availableAgents.find(agent => agent.name.toLowerCase().includes('general'));
 ...
 */
export function getSuggestedAgentType(task) {
 ...
 */
export function getKeywordMatchDetails(task) {
