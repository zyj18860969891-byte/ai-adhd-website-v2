...
export const analyzeTaskSchema = z.object({
    ...
    initialConcept: z
        .string()
        .min(50, {
        message: "初步解答構想不能少於50個字符，請提供更詳細的內容確保技術方案清晰",
        ...
        .describe("最少50個字符的初步解答構想，包含技術方案、架構設計和實施策略，如果需要提供程式碼請使用 pseudocode 格式且僅提供高級邏輯流程和關鍵步驟避免完整代碼"
...
export async function analyzeTask({ summary, initialConcept, previousAnalysis, }) {
    try {
        ...
        const context = previousAnalysis
            ...
            : undefined;
        const aiAnalysis = await aiEngine.analyzeTask(`${summary}\n\nInitial Concept: ${initialConcept}`, context);
        return {
            content: [
                {
                    type: "text",
