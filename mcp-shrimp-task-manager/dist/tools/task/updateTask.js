...
export const updateTaskContentSchema = z.object({
    ...
    relatedFiles: z
        ...
        type: z
            ...
            .describe("文件與任務的關係類型 (TO_MODIFY, REFERENCE, CREATE, DEPENDENCY, OTHER)"
...
export async function updateTaskContent({ taskId, name, description, notes, relatedFiles, dependencies, implementationGuide, verificationCriteria, }) {
    ...
    if (implementationGuide)
        updateSummary += `，更新實現指南`;
    // , update implementation guide
    if (verificationCriteria)
        updateSummary += `，更新驗證標準`;
    ...
    if (relatedFiles && relatedFiles.length > 0) {
        ...
        if (validFiles.length > 0) {
            updateData.relatedFiles = validFiles;
        }
    }
    if (dependencies && dependencies.length > 0)
        updateData.dependencies = dependencies;
    if (implementationGuide)
        updateData.implementationGuide = implementationGuide;
    if (verificationCriteria)
        updateData.verificationCriteria = verificationCriteria;
