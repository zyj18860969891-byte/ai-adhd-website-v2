...
export async function splitTasksRaw({ updateMode, tasksRaw, globalAnalysisResult, }) {
    // 載入可用的代理
    ...
    try {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: "tasksRaw 參數格式錯誤，請確保格式正確，請嘗試修正錯誤，如果文本太長無法順利修復請分批呼叫，這樣可以避免訊息過長導致不好修正問題，錯誤訊息：" +
    ...
    if (!tasksResult.success) {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: "tasks 參數格式錯誤，請確保格式正確，錯誤訊息：" +
    ...
    try {
        ...
        const convertedTasks = tasks.map((task) => {
            ...
            const matchedAgent = availableAgents.length > 0
                ? matchAgentToTask(tempTask, availableAgents)
                : undefined;
            return {
                name: task.name,
                description: task.description,
...
//# sourceMappingURL=splitTasksRaw.js.map