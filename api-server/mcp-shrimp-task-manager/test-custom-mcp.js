...
// 创建测试日志文件
const logStream = fs.createWriteStream('test-custom-mcp.log', { flags: 'w' });
...
async function testCustomMCP() {
  log('Starting Custom MCP Server Test');
  
  // 启动自定义MCP服务（使用编译后的版本）
  const mcpProcess = spawn('node', ['dist/custom-mcp-server.js'], {
  ...
  try {
    ...
    // 关闭MCP服务
    mcpProcess.kill();
    log('Test completed. Check test-custom-mcp.log for detailed logs.');
    logStream.end();
  }
}

// 运行测试
testCustomMCP().catch(error => {
