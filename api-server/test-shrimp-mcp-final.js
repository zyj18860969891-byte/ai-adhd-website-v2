...
async function testShrimpMCPServiceFinal() {
  console.log('🚀 最终完整测试Shrimp MCP服务调用...');
  ...
  try {
    ...
    const successResponses = responses.filter(r => !r.error && r.result && r.result.content && !r.result.content[0].text.includes('Error'));
    const errorResponses = responses.filter(r => r.error || (r.result && r.result.content && r.result.content[0].text.includes('Error')));
    ...
    console.log(`\n🎯 总体成功率: ${successCount}/${toolCalls.length} = ${((successCount / toolCalls.length) * 100).toFixed(1)}%`);
...
// 运行测试
testShrimpMCPServiceFinal().catch(console.error);